...
using Kafka.Ack.Shared.Events;
using Kafka.Ack.Shared.Serializers;


namespace Kafka.Ack.Producer;
...
internal static class KafkaService
{
    ...
    internal static async Task SendMessageToWithAckAsync(string topicName)
    {
        var config = new ProducerConfig()
        {
            BootstrapServers = "localhost:9094",
            //Acks = Acks.None //No ack Status: PossiblyPersisted
            //Acks = Acks.Leader //Leader ack Status: Persisted
            Acks = Acks.All //All ack Status: Persisted
        };
